...
def preprocess(prepare_path, json_write_path):
    ...
    for each_file in prepare_file_list:
        ...
        for each_line in text_read:
            ...
            author = inf_list[1].split('; ')

            institution = inf_list[22]
            citing = inf_list[29]
            di = get_di(inf_list[54], inf_list[55])
            if di:
                di_count += 1
            vl = inf_list[45]
            so = inf_list[9]
    ...
    print('di_inf', di_count, di_count / literature_count)


if __name__ == '__main__':
    prepare_path = '../../data/input/literature'
    json_write_path = '../../data/processed_file/relationship_dict_literature.json'
